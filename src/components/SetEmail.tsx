...
function SetEmail({setAlreadyEmail}) {
  ...
  const insertEmail = async () =>{
          try {
              ...
              if(data[0]?.address || data[0]?.email){
                ...
                let { data,error } = await supabase.from('addresses').update(updates).eq("user_id",user.data.user.id).select();
                if (error) {
                throw error;
                }
                console.log("data after inserting: ",data)
      ...
      <div className="form-control">
        <label className="input-group input-group-vertical">
          ...
          <div className="input-group input-group-horizontal">
            ...
            <button className="btn btn-square" onClick={(e)=>insertEmail()}>
